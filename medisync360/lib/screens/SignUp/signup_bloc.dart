...
class SignupBloc extends Bloc<SignupEvent, SignupState> {
  ...
  SignupBloc({required this.authRepository}) : super(const SignupState()) {
    on<SignupUsernameChanged>((event, emit) {
      emit(state.copyWith(username: event.username, isValid: _validateForm(event.username, state.email, state.password, state.confirmPassword)));
    });

    on<SignupEmailChanged>((event, emit) {
      emit(state.copyWith(email: event.email, isValid: _validateForm(state.username, event.email, state.password, state.confirmPassword)));
    });

    on<SignupPasswordChanged>((event, emit) {
      emit(state.copyWith(password: event.password, isValid: _validateForm(state.username, state.email, event.password, state.confirmPassword)));
    });

    on<SignupConfirmPasswordChanged>((event, emit) {
      emit(state.copyWith(confirmPassword: event.confirmPassword, isValid: _validateForm(state.username, state.email, state.password, event.confirmPassword)));
    });
  ...
  Future<void> _onFetchLocation(FetchLocationRequested event, Emitter<SignupState> emit) async {
    try {
      ...
      emit(state.copyWith(latitude: position.latitude, longitude: position.longitude, isSubmitting: false));
    } catch (e) {
  ...
  Future<void> _onSignupSubmitted(SignupSubmitted event, Emitter<SignupState> emit) async {
    ...
    try {
      ...
      if (response['success'] == true) {
        ...
        emit(state.copyWith(isSubmitting: false, errorMessage: response['message'] ?? "Signup failed"));
      }
    } catch (e) {
